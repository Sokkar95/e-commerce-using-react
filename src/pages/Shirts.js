...
function Shirts({ s_cont, addToCart }) {
  const [name, setName] = useState("");

  // the search result
  const [foundItems, setfoundItems] = useState(s_cont.shirts);

  const filter = (e) => {
    const keyword = e.target.value;

    if (keyword !== "") {
      const results = s_cont.shirts.filter((item) => {
        return item.name.toLowerCase().startsWith(keyword.toLowerCase());
        // Use the toLowerCase() method to make it case-insensitive
      });
      setfoundItems(results);
    } else {
      setfoundItems(s_cont.shirts);
