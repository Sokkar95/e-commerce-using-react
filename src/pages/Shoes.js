...
function Shoes({ sh_cont, addToCart }) {
  const [name, setName] = useState("");

  // the search result
  const [foundItems, setfoundItems] = useState(sh_cont.shoes);

  const filter = (e) => {
    const keyword = e.target.value;

    if (keyword !== "") {
      const results = sh_cont.shoes.filter((item) => {
        return item.name.toLowerCase().startsWith(keyword.toLowerCase());
        // Use the toLowerCase() method to make it case-insensitive
      });
      setfoundItems(results);
    } else {
      setfoundItems(sh_cont.shoes);
