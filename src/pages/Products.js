...
function Products({ addToCart, cont }) {
  // const randomProducts=cont.products.sort(()=>0.5-Math.random())

  const [name, setName] = useState("");

  // the search result
  const [foundItems, setfoundItems] = useState(cont.products);

  const filter = (e) => {
    const keyword = e.target.value;

    if (keyword !== "") {
      const results = cont.products.filter((item) => {
        return item.name.toLowerCase().startsWith(keyword.toLowerCase());
        // Use the toLowerCase() method to make it case-insensitive
      });
      setfoundItems(results);
    } else {
      setfoundItems(cont.products);
            ...
            foundItems.map((item) => (
              <div key={item.id} className="col-md-4">
                <div className="item" style={{ textAlign: "center" }}>
                  <img height="325" className="w-100" src={item.image} />
